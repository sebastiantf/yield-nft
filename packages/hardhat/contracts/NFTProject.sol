...
contract NFTProject {
    ...
    INFT20Factory private constant NFT20Factory = INFT20Factory(0x0f4676178b5c53Ae0a655f1B19A96387E4b8B5f2); // proxy for NFT20FactoryV4: 0x1813C4485a36642347c244a00617377BAFdBE401
    ...
    function isValidBorrowerSignature(
        ...
        bytes32 message = keccak256(abi.encodePacked(_nftTokenContract, _nftTokenId, _borrower, _strategy, _yVault, _viaNFT20));
    ...
    function isValidLenderSignature(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(_lendPrincipalAmount, _lendERC20Contract, _nftTokenContract, _nftTokenId, _closingTime, _lender)
        );
        bytes32 messageWithEthSignPrefix = message.toEthSignedMessageHash();
        address signer = messageWithEthSignPrefix.recover(_signature);
    ...
    function beginInvestment(Investment memory _investment) public returns (uint256, uint256) {
        ...
        if (_investment.viaNFT20) {
            ...
            IERC721(_investment.nftTokenContract).transferFrom(msg.sender, _investment.strategy, _investment.nftTokenId);
    ...
    function endInvestment(uint256 _investId) public returns (uint256, uint256) {
        ...
        if (investment.viaNFT20) {
            ...
            console.log("nft20pair balance after swap: ", IERC20(NFT20Factory.nftToToken(investment.nftTokenContract)).balanceOf(address(this)));
            uint256[] memory tokenIds = new uint256[](1);
            tokenIds[0] = investment.nftTokenId;
            uint256[] memory amounts = new uint256[](1);
            amounts[0] = 1;
            NFT20Pair.withdraw(tokenIds, amounts, investment.borrower);
            IERC20(investment.lendERC20Contract).transfer(investment.borrower, IERC20(investment.lendERC20Contract).balanceOf(address(this)));
        }
    ...
    function _swap(
        ...
        if (_forExact) {
            tokensSwapped = UniswapV2Router02.swapTokensForExactTokens(amount, amounts[0], path, address(this), deadline)[path.length - 1];
        } else {
            tokensSwapped = UniswapV2Router02.swapExactTokensForTokens(amount, 1, path, address(this), deadline)[path.length - 1];
        }
